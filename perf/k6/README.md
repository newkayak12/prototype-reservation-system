...
| `run.sh` | `seed.sh` → `k6 run`(raw JSON 타임시리즈 출력 포함) → `analyze-vu-stages.py` → `wait-settle.sh` → `verify-overbooking.sql`을 10회(`RUNS` env로 조절, `START` env로 재개 지점 지정 가능) 반복하고 결과를 `results/`에 저장. `./run.sh <scenario-label>` (예: `baseline`, `redesigned`)로 호출. |
| `lib/analyze-vu-stages.py` | k6의 `--out json` raw 타임시리즈에서 `http_req_duration` 포인트를 `vu_stage` 태그(booking.js가 요청 시점의 실제 활성 VU를 100/300/600/1000/1500/2000 중 가장 가까운 값으로 스냅해 붙임)로 묶어, VU 단계별 요청수/처리량(req/s)/상태코드 분포/p50·p95·p99 레이턴시를 표로 출력. 분석 후 raw JSON은 용량 때문에 삭제하고 이 표만 `results/`에 남긴다. |
...
| `results/` | 각 run의 k6 summary JSON/log, VU 스테이지 breakdown, settle 결과, 오버부킹 검증 결과 (gitignore). |
...
# 중간에 끊긴 경우 앞의 결과를 지우지 않고 이어서 실행
START=4 RUNS=10 ./perf/k6/run.sh baseline
```

결과는 `perf/k6/results/<label>-<n>.json`(k6 summary), `<label>-<n>.log`, `<label>-<n>-vu-stages.txt`
(VU 단계별 처리량/레이턴시), `<label>-<n>-settle.json`, `<label>-<n>-overbooking.txt`로 쌓인다.
`docs/perf/baseline-report.md` / `baseline-vs-redesign.md`는 이 산출물들을 집계해서 사람이 읽을 수 있게
정리한 문서다.

## 측정 항목과 Phase 0에서 확인된 한계

- **HTTP 레벨**: p50/p95/p99, 상태코드별 카운트(성공/품절/충돌/5xx) — k6 자체 출력.
- **VU 단계별 처리량/레이턴시**: `analyze-vu-stages.py`가 만드는 표. "VU를 올리면서 처리량·부하·처리속도가
  어떻게 변하는가"를 한눈에 보여준다 — 예: VU 100 구간에서는 초당 수십 건이 성공하다가 VU 600 이상부터는
  성공 건수가 0에 수렴하고 p95 레이턴시가 60초(k6 기본 타임아웃)에 그대로 눌러붙는 식. 집계 총합만으로는
  안 보이던 "몇 VU부터 무너지는가"가 이 표에서 바로 드러난다.
- **트래픽 포화점**: 위 VU 단계별 표에서 성공률/레이턴시가 급격히 꺾이는 지점.
