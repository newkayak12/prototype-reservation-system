...
// ReservationFailed는 contract-module이 발행하는 reservation 통합 이벤트의 JSON 와이어 형태를 고정한다.
...
class ReservationFailedEventJsonTest : BehaviorSpec({

    val fixedOccurredAt: Instant = Instant.parse("2024-03-15T09:30:45.123456789Z")

    fun reservationFailed(occurredAt: Instant = fixedOccurredAt) =
        ReservationFailed(
            ...
            traceparent = "00-trace-03",
        )

    val goldenJson =
        """
        {"eventId":"33333333-3333-3333-3333-333333333333","aggregateType":"Reservation","aggregateId":"reservation-3","sequenceNo":3,"eventType":"ReservationFailed","eventVersion":1,"occurredAt":"2024-03-15T09:30:45.123456789Z","correlationId":"correlation-3","causationId":"causation-3","traceparent":"00-trace-03"}
        """.trimIndent()

    given("ReservationFailed 통합 이벤트가 주어졌을 때") {
        `when`("EventJson으로 직렬화하면") {
            val json = EventJson.encode(reservationFailed())
        ...
        `when`("직렬화 후 같은 타입으로 역직렬화하면") {
            val original = reservationFailed()
            val json = EventJson.encode(original)
            val restored = EventJson.decode<ReservationFailed>(json)
        ...
        `when`("nanoAdjustment가 0인 Instant로 왕복하면") {
            val original = reservationFailed(occurredAt = Instant.parse("2024-03-15T09:30:45Z"))
            val restored = EventJson.decode<ReservationFailed>(EventJson.encode(original))
