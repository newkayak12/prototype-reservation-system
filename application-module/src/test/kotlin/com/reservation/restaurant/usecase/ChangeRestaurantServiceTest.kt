...
class ChangeRestaurantServiceTest {
    @Nested
    @DisplayName("존재하지 않는 레스토랑 ID가 주어졌을 때")
    inner class `Context with non existent restaurant id` {
        @MockK
        private lateinit var changeRestaurantDomainService: ChangeRestaurantDomainService
        ...
        @InjectMockKs
        private lateinit var useCase: ChangeRestaurantService
        ...
        fun `it throws NoSuchPersistedElementException`() {
            ...
            verify(exactly = 0) {
                changeRestaurantDomainService.change(any(), any())
    ...
    inner class `Context with failed photo upload` {
        ...
        fun `it throws an exception and does not update the restaurant`() {
            ...
            verify(exactly = 0) { changeRestaurantDomainService.change(any(), any()) }
        }
    }
    ...
    inner class `Context with failed database update` {
        ...
        fun `it throws an exception and rolls back the changes`() {
            ...
            every { changeRestaurantDomainService.change(any(), any()) } returns restaurantSnapshot
