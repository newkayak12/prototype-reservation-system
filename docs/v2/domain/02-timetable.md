...
- **행위**: `attachOccupied(userId)` — 상태를 OCCUPIED로, Occupancy 생성. `detachOccupied()` — EMPTY로, Occupancy unoccupied.
- **도메인 서비스**: `CreateTimeTableOccupancyDomainService` (검증+점유), `CreateTimeTableOccupiedDomainEventService` (이벤트 생성)
