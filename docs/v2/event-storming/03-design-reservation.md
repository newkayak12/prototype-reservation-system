...
| 4 | `PENDING → EXPIRED`: ExpireReservation (결제 대기 만료) | 시간 경과·스케줄러 (**reservation 자체 스케줄러**가 결제 대기 만료 판정 — 타임아웃 소유권 이전 [[ADR-008-saga-orchestration-vs-choreography]] 개정; reservation이 먼저 EXPIRED로 전이한 뒤 `ReservationExpired`를 발행해 timetable이 좌석 해제) | `Proposed`([[ADR-008-saga-orchestration-vs-choreography]]) | `V2 도메인 문서 근거`(domain/01 §2 · ADR-008 개정 · [[DESIGN-007-consistency-and-sagas]] §4.4) |
...
| `ReservationExpired` | `ExpireReservation`(결제 대기 만료, reservation 스케줄러 — [[ADR-008-saga-orchestration-vs-choreography]] 개정) | 시간 경과·스케줄러 (reservation 자체) | `ExpireReservation` | Reservation | `reservationId, expiredAt` — timetable이 이를 구독해 좌석 해제(`ReleaseSeat → SeatReleased`) | domain/01 §2 · [[ADR-008-saga-orchestration-vs-choreography]] 개정(타임아웃=reservation 소유) — V1 대응 없음(순수 신설) | `Proposed`([[ADR-008-saga-orchestration-vs-choreography]]) | `V2 도메인 문서 근거` |
...
| 5 | 참조 ID(`userId`·`restaurantId`·`slotId`) 비어있지 않음 + UUID v7 포맷 | `V1 코드에서 확인` | `—`(V1 코드, ADR/RFC 아님) | V1 validation 정책 8개(4 ID × {Empty, Format}) 승계 — 예: `core-module/src/main/kotlin/com/reservation/reservation/policy/validations/ReservationUserIdEmptyValidationPolicy.kt`·`ReservationUserIdFormatValidationPolicy.kt`(디렉터리 `.../policy/validations/`는 4 ID × 3파일 = 12개 파일이며, 그중 각 ID의 Empty/Format 구현 8개가 동일 패턴 — 나머지 `...IdPolicy.kt` 4개는 정책이 아니라 각 구현이 따르는 **인터페이스**). ID 포맷은 도메인 의도상 v4→v7이나, Format 정책 정규식(`^[0-9a-fA-F]{8}-...`)은 버전 니블을 제한하지 않아 v7도 그대로 통과한다. 서비스 검증은 `core-module/src/main/kotlin/com/reservation/reservation/service/validate/ValidateUserId.kt`(디렉터리 `.../service/validate/` 안 4개 전부 동일 패턴) |
