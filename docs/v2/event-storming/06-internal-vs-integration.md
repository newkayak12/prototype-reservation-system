...
## 3. 분류표 — timetable·reservation 카탈로그 이벤트명 전부 (고유 이벤트명 14개 · 표 행 14행, 미분류 0건)

타임아웃 소유권 이전([[ADR-008-saga-orchestration-vs-choreography]] 개정 · [[07-hotspots-and-open-questions]] H1) 이후 `SeatReleased`는 5a/5b 분리 없이 **단일 내부 이벤트**다 — 이름 수(14) = 행 수(14). 과거 "한 이름이 두 분류에 걸침" 문제의 소멸은 아래 §4가 기록한다.
...
|---|---|---|---|---|---|
| `SlotProvisioned` | timetable | 내부 도메인 이벤트 | — | [[04-policies-and-choreography]] 표·그래프 어디에도 등장하지 않음. 07 Q3·H3(2026-07-31)으로 존재·이름이 확정됐고, "존재한다면 내부"라는 분류가 이제 확정 이벤트에 그대로 적용된다 | `V2 도메인 문서 근거`([[07-hotspots-and-open-questions]] Q3·H3, 2026-07-31 채택) |
...
| `SeatReleased` | timetable | 내부 도메인 이벤트 | — | 타임아웃 소유권 이전 이후 `SeatReleased`는 취소·실패·노쇼·**만료** 모든 경로에서 timetable이 reservation 이벤트를 구독해 낸 내부 결과다(P6·P8·P10·P13 어디에도 `SeatReleased`를 트리거로 삼는 후속 정책 없음). 과거 TTL 유래(5b)가 reservation으로 향하던 통합 엣지는 `ReservationExpired`(reservation→timetable)로 대체돼 사라졌다(§4) | `V2 도메인 문서 근거` |
...
| `ReservationExpired` | reservation | 통합 이벤트 | `timetable`(P8) | 타임아웃 소유권 이전([[ADR-008-saga-orchestration-vs-choreography]] 개정) 이후 P8: "`ReservationExpired`(reservation) → `ReleaseSeat`(timetable) → `SeatReleased`". 과거엔 timetable-선행이라 종착 내부 결과였으나, 이제 reservation-선행이 되며 timetable이 구독하는 통합 이벤트다 | `V2 도메인 문서 근거` |
...
**집계**: 고유 이벤트명 14개(표 행수도 14행 — 타임아웃 소유권 이전으로 `SeatReleased` 5a/5b 분리가 소멸) 전부가 내부/통합 중 하나로 분류됐다 — 미분류는 0건이다. `SeatReleased`는 이제 단일 내부 이벤트이고, 과거 "같은 이름이 두 분류에 걸치는" 유일 사례였던 문제는 §4가 기록한 대로 경로 통일로 해소됐다.

---

## 4. `SeatReleased` — 단일 내부 이벤트로 수렴 (타임아웃 소유권 이전으로 dissolve)

[[02-design-timetable]] §4는 원래 `SeatReleased`가 원인이 다른 두 트리거(5a 외부 이벤트/5b TTL)를 동일 이름으로 공유하는 것을 미결로 남겼고, 이 문서도 그 이름이 5a=내부·5b=통합 두 분류에 걸치는 문제를 §3에서 짚었다.

**해소 (2026-07-31, [[ADR-008-saga-orchestration-vs-choreography]] 개정 · [[07-hotspots-and-open-questions]] H1)**: 타임아웃 소유권을 `timetable`에서 `reservation`으로 옮겼다. 이제 결제 대기 만료도 `reservation`이 자기 스케줄러로 판정해 `ReservationExpired`를 발행하고, `timetable`은 이를 취소·실패·노쇼와 **동일 경로**로 구독해 `ReleaseSeat → SeatReleased`한다. 그 결과:

- timetable의 `ExpireSeat`·자체 TTL이 사라져 5b(TTL 유래 `SeatReleased`) 자체가 없어진다.
- `SeatReleased`는 모든 경로에서 **timetable 내부 단일 이벤트**로 수렴한다 — cross-context 소비자가 없다(§3).
- 과거 5b가 reservation으로 향하던 통합 엣지는 `ReservationExpired`(reservation→timetable)가 대신한다(§3 · [[04-policies-and-choreography]] §3).

즉 "하나의 이름이 두 분류에 걸침" 문제는 **이름 분리가 아니라 경로 통일로 소멸**했다. Phase 7-1은 `SeatReleased`를 내부 이벤트로만 다루면 되고 계약 모듈로 올릴 필요가 없다. 원인 미구분(과거 `cause` 필드 미결)도 함께 해소된다 — 원인은 timetable이 어느 reservation 이벤트를 구독했는지로 이미 나뉘므로 페이로드 필드가 불필요하다.
...
|---|---|---|---|
| `TimeTableCreated`(timetable 패키지 주석) | [[02-design-timetable]] §3의 슬롯 생성 이벤트는 07 H3(2026-07-31)으로 `SlotProvisioned` 확정, `TimeTableCreated`는 폐기 결정 | **해소** — 07 H3(2026-07-31) `SlotProvisioned` 확정으로 이름 표류 종결. modules/02 §4 예시 주석 정정은 07 전파 대기 배치로 | `V2 도메인 문서 근거`([[07-hotspots-and-open-questions]] H3, 2026-07-31 확정) |
...
§3에서 통합 이벤트로 판정된 이벤트를 이관 후보로 제시한다. 타임아웃 소유권 이전(§4) 이후 `SeatReleased`는 단일 내부 이벤트라 이관 대상이 아니고, 그 자리를 `ReservationExpired`(reservation→timetable 통합)가 대신한다.

| 카탈로그 명명 | 소비 컨텍스트 | 이관 후보 | 근거 | 태그 |
|---|---|---|---|---|
| `SeatHeld` | payment(범위 밖) | 후보 | §3 | `V2 도메인 문서 근거` |
| `ReservationExpired` | timetable | 후보 | §3·§4 — 타임아웃 소유권 이전으로 통합이 된 이벤트, 과거 `SeatReleased`(5b)를 대체 | `V2 도메인 문서 근거` |
...
- ~~`SeatReleased` 5a(내부)·5b(통합) 이름 공유로 인한 이관 경계 모호성~~ **해소됨(2026-07-31, §4)** — 타임아웃 소유권을 reservation으로 옮겨 5b(TTL 유래) 경로가 사라지고 `SeatReleased`가 단일 내부 이벤트로 수렴, 이관 경계 모호성이 소멸했다([[ADR-008-saga-orchestration-vs-choreography]] 개정 · [[07-hotspots-and-open-questions]] H1).
- §6.1의 페이로드 씬닝 지점 — 위치만 표시했고 실제 필드 축소 여부는 미판단.
- ~~§5의 `TimeTableCreated` 어긋남~~ **해소됨(2026-07-31, §5)** — 07 H3으로 슬롯 생성 이벤트 이름이 `SlotProvisioned`로 확정되고 `TimeTableCreated`(modules/02 예시 주석)는 폐기 결정됐다([[07-hotspots-and-open-questions]] H3). modules/02 §4 예시 주석 정정은 07 전파 대기 배치로 미룬다.
