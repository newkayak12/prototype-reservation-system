...
- **상태**: ✅ 종결 (2026-06-28) · 하류 산출물 없음 — V2는 클린 슬레이트이므로 이행 자체가 불필요. 스냅샷으로 잡고 다음 이벤트부터 채우면 되는 한 줄짜리 문제를 과잉 분석한 것이라는 지적으로 닫음.
...
| 용어 | 정의 |
|------|------|
| **제네시스 이벤트 (Genesis Event)** | 이벤트 이력이 없는 기존 데이터를 이벤트 스토어에 태울 때, 현재 상태를 통째로 싣는 첫 번째이자 유일한 시작 이벤트. "이 aggregate의 이력은 여기서부터 시작한다"는 선언. 성경의 Genesis(창세기)에서 따온 이름. |
| **컷오버 (Cutover)** | V1→V2 전환에서 실제 트래픽을 구 시스템에서 신 시스템으로 넘기는 전환점. 피처 플래그 토글, DNS 변경, 라우팅 룰 변경 등 방식은 다양하지만 핵심은 "스위치를 누르는 그 순간". |
...
| # | 결정 | ADR |
|---|------|-----|
| 0 | **V2는 V1 데이터를 계승하지 않는다(클린 슬레이트)** — 아래 시딩·병행 전략은 현재 적용 범위 밖, 향후 계승 시 단일 스냅샷(genesis (a))이 기본 | — |
| 1 | V1 행 시딩 = **genesis 단일 이벤트 (a) 기본 + 컨텍스트별 (b) 합성 이력 예외**, genesis도 스키마·버전 보유 | — |
| 2 | 병행 창 = **단방향 동기 + 긴 병행(무중단 동기 + 플래그 컷오버) 기본**, 레퍼런스 한 바퀴 후 잎 컨텍스트만 얇은 컷오버 | — |
| 3 | 백필 = **[[RFC-011-projection-rebuild-catchup]] 재구축/캐치업 재사용**, 별도 파이프라인 없음 | — |
| 4 | 컷오버·롤백 = **피처 플래그 토글 + V1 보존 기간**, 트래픽 롤백 즉시·데이터 롤백은 시간 흡수 | — |
| 5 | 검증 = **동등성 3축(건수·핵심 필드·재구성 상태)을 컷오버 게이트로**, 회귀로 고정 | — |
