...
| **축 구분** | — | wire(같은 시점) ≠ 업캐스팅(시간 축) | "지금 생산자 변경이 지금 소비자를 깨나(이 RFC) vs 과거 이벤트를 새 코드로 읽나([[RFC-023-event-schema-evolution]])" |
...
- **이 RFC가 다루는 건 wire 축이다.** 업캐스팅([[RFC-023-event-schema-evolution]])은 *과거* 이벤트를 새 코드로 읽는 *시간 축* 호환이고, 여기 계약은 *같은 시점에* 생산자 변경이 소비자를 깨지 않는지를 본다. → 둘 다 필요하지만 결이 다르므로 섞으면 안 된다. [[RFC-004-event-store-schema-evolution]]은 저장·보존(시간 축에서 저장소를 어떻게 운영하나)이라 또 다른 축이다.
...
**Non-goal (이번에 하지 않음)**
- 과거 이벤트를 새 코드로 읽는 업캐스팅(시간 축 호환). → [[RFC-023-event-schema-evolution]].
...
- 이웃 축: [[RFC-023-event-schema-evolution]] · [[RFC-004-event-store-schema-evolution]]
