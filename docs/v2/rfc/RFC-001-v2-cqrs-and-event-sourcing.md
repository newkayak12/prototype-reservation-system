...
본 RFC(라운드1)는 위 7개 결정의 **방향·메커니즘**만 실제로 논의해 확정했다. 그 아래 구체 결정 — 읽기 일관성 기준, 도메인 이벤트 카탈로그, 신규 기능, 스냅샷 주기, 파티션·키 운영, PII 키 운영 등 — 은 **여기서 논의하지 않았다.** 흩어진 미결 전체는 [[RFC-002-decision-queue]]에 주제별 RFC(RFC-003~011)로 모여 하나씩 닫힌다.
