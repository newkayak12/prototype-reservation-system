...
> **추가 (2026-07-05, 트리아지 C10 검증 축)**: "사가 보상·타임아웃" 범주에 **코레오그래피 가드 맞물림 검증**을 포함한다 — 컨텍스트별로 흩어진 상태 가드(예: `reservation`이 EXPIRED에서 `PaymentConfirmed` 확정 거부, [[DESIGN-007-consistency-and-sagas]] §4.7라)가 서로 맞물려 **오버부킹 전역 불변식**을 지키는지 다중 컨텍스트 통합 시나리오로 검증. 단일 슬롯 오버부킹의 구조적 백스톱은 timetable 단일 애그리거트 직렬화 + L0 UNIQUE([[DESIGN-006-aggregate-design]])이고, 이 테스트는 그 위에서 *가드 로직의 정합*을 본다. 게이트=정기/통합.
