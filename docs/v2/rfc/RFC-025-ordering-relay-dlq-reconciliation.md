# RFC-025 — aggregate별 순서 계약과 relay·DLQ의 상호작용 봉합 (단일 순차 relay · Last-Writer-Wins 가드 · DLQ=알림/재구축)

- **상태**: 🏷 합의 (2026-07-04) — 단일 순차 relay(ShedLock) · LWW seq 가드 · DLQ는 되쏘지 않고 event_store 재구축으로 복구 · 순서 결정적 소비는 꼬리 격리. ADR 비준 대기
...
| # | 결정 | 상태 | ADR |
|---|------|------|-----|
| 1 | relay = **단일 순차(ShedLock 리더)**, Kafka 프로듀서 idempotent/ordered — SKIP LOCKED 경쟁 supersede. CDC·파티션드는 졸업 후보 | 결정 | 신규 ADR 예정 · [[DESIGN-008-messaging-topology]] |
| 2 | 프로젝션 = **Last-Writer-Wins seq 가드**(superseded 무시·최신 적용); commutative는 dedup만 | 결정 | 신규 ADR 예정 · [[RFC-021-event-identity-and-global-ordering]] 정합 |
| 3 | **DLQ는 라이브 스트림에 되쏘지 않음** — 알림/감사 로그, 복구는 event_store 재구축([[RFC-011-projection-rebuild-catchup]]) — 수동 재생 supersede | 결정 | 신규 ADR 예정 · [[DESIGN-008-messaging-topology]] |
| 4 | 순서 결정적 소비(사가·부수효과·비가환) = **바운드 재시도 → aggregate 꼬리 격리**(적용 보장) | 결정 | 신규 ADR 예정 · [[DESIGN-006-aggregate-design]]·[[DESIGN-007-consistency-and-sagas]] |
| 5 | inbox에 **aggregate별 last-applied `sequence_no`** 추가(2·4의 토대) | 결정 | 신규 ADR 예정 |
...
```mermaid
graph LR
    OBX[("outbox")] -->|단일 순차 relay<br/>ShedLock| K[("Kafka<br/>partition=aggId")]
    K --> PJ["프로젝션<br/>LWW seq 가드"]
    K --> SG["사가·부수효과<br/>재시도→꼬리 격리"]
    PJ -. 실패 .-> DLQ[("DLQ<br/>알림/감사")]
    SG -. 실패 .-> DLQ
    ES[("event_store")] -.복구=재구축.-> PJ
```

- **발행**: 단일 순차 relay가 aggregate 순서를 보존. (처리량 문제 시 파티션드/CDC 졸업.)
- **프로젝션**: LWW seq 가드로 재정렬 자가치유 — 옛 이벤트는 무시, 최신만 적용.
- **DLQ**: 되쏘지 않음. 복구는 event_store 재구축(seq 순서 보장).
- **순서 결정적 소비**: 꼬리 격리로 앞 순서 미적용 원천 차단, 무관 aggregate는 안 막음.
