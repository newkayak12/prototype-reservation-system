...
- 재판단하니 자리를 남이 이미 선점해 도메인 상태가 바뀐 거절 → **422**로 클라에 알림(확정 거절)

남는 충돌 표면 셋:
- **lock-wait 타임아웃** → 재시도 신호(409/503 + retry-after)
- **도메인 거절**(락 잡고 reload하니 "자리를 남이 선점함") → **422**, 확정적 거절
- **잔여 UNIQUE 위반**(락 유실 edge) → 동시성 충돌로 흡수(바운디드 재시도/409)

> **⚠ 정합화 (2026-07-07, 트리아지 C36 ②)** — 판별축을 "도메인 판단이 *뒤집히느냐*"에서 **"도메인 상태가 실제로 커밋되어 바뀌었느냐(선점)"**로 다듬어 확정([[12-api-contract]] §4.4). 자리 경합으로 자리가 없어진 것은 경합의 결과지만 그 자리가 돌아오지 않으므로 `422`(이 절 초안이 자리 경합을 409로 적었던 것은 정정). `409`는 순수 락 경합(lock-wait 타임아웃·버전 충돌·락 유실 UNIQUE 백스톱, 재시도로 풀림)에 한정.
