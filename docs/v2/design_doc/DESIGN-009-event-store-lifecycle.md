...
- **"스냅샷 폐기 후 리플레이 재생성"이 콜드 파티션과 충돌** — §4.2·§5.2는 스냅샷을 업캐스팅하지 않고 schema_version 불일치 시 폐기→이벤트 리플레이로 재생성한다. 그런데 §4.2·§6.3은 닫힌 파티션을 export 후 drop해 S3로 이관한다. 장수 애그리거트(§5.3·§7이 인정)의 스냅샷을 스키마 변경 후 폐기하면, 재생성에 필요한 옛 이벤트가 이미 콜드로 빠져 hot DB에 없을 수 있다 — 이 경우 "안전하고 단순한 재생성"이 실은 S3 아카이브 리플레이(§4.4 "드물게")를 핫 경로 스키마 마이그레이션에서 강제 호출하게 된다. 스냅샷 폐기의 단순성이 콜드 이관의 전제(옛 이벤트는 거의 안 읽힘)를 깨는 이 상호작용이 문서에 없다.

- **읽기 시점 업캐스팅은 리플레이마다 반복 비용을 지불** — §4.2·ADR-10은 in-place 마이그레이션을 기각하고 읽기 시 업캐스팅을 택한다. 이는 모든 애그리거트 로드·모든 프로젝션 재구축·모든 as-of 조회가 옛 이벤트를 만날 때마다 업캐스터 체인을 재실행한다는 뜻이다. v1→v2→v3로 스키마가 누적되면 가장 오래된 이벤트는 매 읽기마다 2단 체인을 타고, 프로젝션 전체 재구축(§6.3, event_store 전체 스캔)은 이 비용을 수백만 이벤트에 곱한다. §6.3의 p99>50ms 가드레일은 스냅샷+증분만 보고 업캐스팅 누적 비용을 재구축 경로에서 계산하지 않았다.

- **N=100 기본값은 예약 도메인에서 사실상 스냅샷 무력화** — §6.3은 N=100이고 "예약 애그리거트는 스트림이 짧아 스냅샷이 사실상 거의 안 찍힌다 — 의도다"라고 한다. 그렇다면 §4의 스냅샷 저장 모델·2-슬롯 토글·Glacier 이관·reconciliation 배치 전체가 거의 발동하지 않는 기능에 대한 인프라 투자가 된다. 짧은 스트림이면 seq 0 리플레이가 이미 싸므로 스냅샷이 불필요하고, 스냅샷이 유의미할 만큼 긴 스트림(장수 애그리거트)에서는 §5.3·§7이 파티션 경계 문제를 인정한다. 즉 스냅샷이 값싼 곳엔 불필요하고 필요한 곳엔 위험하다 — N=100은 이 딜레마를 "의도"로 포장한다.

- **크립토 셰딩 백업 비대칭이 복구 완전성과 상충** — §6.2는 "이벤트 백업은 암호문만, 키 백업은 셰딩 의미론 보존(셰딩된 키는 백업에서 무효/제외)"으로 셰딩을 지킨다. 그런데 §4.2·G4는 이벤트 스토어를 "재해 복구 1급 대상 한 곳"으로 삼고 "이벤트만 있으면 read model 재구축 가능"이라 한다. 키가 셰딩된 PII 필드는 복구 후 영구 복호 불가이므로, 그 PII에 의존하던 read model 컬럼·프로젝션은 재구축해도 빈 값이 된다. "이벤트 스토어 한 곳이면 완전 복구"라는 DR 서사와 "셰딩된 것은 부활하지 않는다"는 PII 서사가 같은 문서에서 충돌하는데, 복구 후 read model이 부분적으로 불완전해진다는 사실이 복구 순서 불변식(§4.3)에 반영되지 않았다.

- **temporal 조회 ops 한정이 감사 요구와 모순 소지** — §4.2·§6.3은 as-of 조회를 "운영·디버깅 한정(YAGNI)"으로 API 비노출한다. 그러나 §1은 ES의 가치로 "감사"를 명시하고 §9.2는 "종결+90일 감사·분쟁 윈도우"를 둔다. 분쟁·감사는 대개 *비개발자*(CS·법무·규제 대응)가 "그 시점 상태"를 물어야 하는데, ops 한정이면 이들이 매번 개발자 수동 조회에 의존한다. temporal을 YAGNI로 미루면서 감사성을 ES 채택의 정당화로 드는 것은, 감사 요구가 증명되면 그때 프로젝션화한다는 사후 대응인데 감사·규제는 사전 준비가 요구되는 영역이라 "요구 증명 후 대응"이 늦을 수 있다.

- **reconciliation 배치가 "진실"로 seq 0 전체 리플레이를 씀 — 자기모순** — §9.1은 reconciliation을 "스냅샷+증분 결과와 seq 0 전체 리플레이 결과를 비교"로 정의한다. 그런데 §6.3은 "핫 경로에서 seq 0 전체 리플레이가 일어나면 안 된다"를 가드레일로 못 박는다. reconciliation이 정합성을 검증하려면 seq 0 전체 리플레이를 정기적으로 수행해야 하고, 이는 콜드로 이관된 파티션까지 포함해야 완전한데(§6.3은 "콜드 포함 여부 선택"), 콜드를 빼면 검증이 부분적이고 포함하면 S3 리플레이를 정기 배치로 상시화한다. reconciliation의 신뢰도와 그 비용이 서로를 잠식하는 트레이드오프가 표본률 "운영 측정에 맡김"(§9.2)으로 얼버무려졌다.

- **논리 사고를 보상 이벤트로만 정정하는 원칙의 사각지대** — §4.2·§5.5·§9.1은 잘못된 이벤트 주입·논리 사고를 절단이 아닌 보상 이벤트로 정정한다. 이는 "다운스트림이 이미 봤다"는 이유로 정당하지만, *민감정보가 잘못 주입된 경우*(예: 타인의 PII가 엉뚱한 aggregate에 append)에는 보상 이벤트가 원 이벤트를 논리적으로 무효화할 뿐 암호문 자체는 event_store에 영구 잔존한다. 크립토 셰딩(§6.2)은 키 단위 삭제라 aggregate 단위 오주입 정정과 입도가 안 맞을 수 있다. append-only 불변성을 지키는 대가로, 잘못 들어간 개인정보를 물리적으로 제거할 경로가 이 문서 어디에도 없다.
