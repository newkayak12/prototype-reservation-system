...
- **크래시 후 재구성·재개(rehydrate/resume).** PM의 상태가 ES 스트림이라는 선택은 단순한 추적 편의가 아니라 *복구 정확성*의 근거다 — PM 프로세스가 중간에 죽어도, 다시 뜰 때 사가 스트림(`SeatHeld`…)을 재생해 "어디까지 왔는지"를 그대로 복원하고 끊긴 단계부터 이어 간다. 다음 단계를 진행했는지 여부가 스트림에 남아 있으므로, 위 consume-then-emit 멱등과 합쳐 재개 시 중복 진행을 막는다.
- **열린 사가 조회용 read model.** 폴링 스케줄러가 "기한 지난 미완 사가/점유"를 깨우려면(앞의 사가 타임아웃 절), *deadline으로 열린 사가를 찾는* 질의가 싸야 한다. ES 스트림은 추적·복구엔 좋지만 "deadline 지난 열린 사가 전부"를 훑기엔 부적합하므로, 이 폴링을 받칠 **열린 사가 read model**(사가 id·현재 단계·deadline·부분 보상 진행)을 둔다 — 사가 이벤트로 갱신되는 프로젝션이며, 사가가 닫히면(`ReservationConfirmed`/`SeatReleased`) 그 항목이 빠진다. 부분 보상 추적도 여기에 함께 남아 위 PoisonMessage 운영 흐름과 맞물린다. 구체 스키마는 구현 사이클에서 확정(TBD).
...
- **여러 단계를 보상할 때는 정방향의 역순으로 되감는다.** 점유→결제까지 진행한 사가가 확정에서 깨지면 마지막에 일어난 일(결제)을 먼저 되돌리고(`PaymentRefunded`) 그 앞(점유)을 나중에 되돌린다(`SeatReleased`). 분산 트랜잭션엔 다단계 보상을 한 번에 묶을 원자성이 없으므로, PM은 *어느 보상까지 돌렸는지*를 사가 스트림(앞 절 ES 상태)에 단계별로 남기고 중단된 지점부터 이어서 보상한다 — 한 보상이 실패해도 이미 돈 보상이 두 번 돌지 않게.
- **consume-then-emit 멱등.** PM이 한 단계 이벤트를 소비하고 다음 커맨드를 내는 한 묶음은, 같은 입력 이벤트를 두 번 받아도 같은 커맨드를 두 번 내지 않아야 한다(이미 그 단계를 진행한 사가면 무시). 사가 스트림에 "이 단계는 이미 처리됨"이 남아 있으므로, 재전달·재처리 시 스트림을 기준으로 중복 진행을 거른다.
...
### (다) 두 시계가 충돌할 때 — 만료된 점유에 확정이 닿는 레이스

(가)와 (나)는 *서로 다른 시계*다 — 점유 만료는 `timetable` 애그리거트가, 사가 스텝 타임아웃은 PM이 본다. 두 시계가 폴링으로 굴러가므로 정밀도는 폴링 주기만큼 거칠고, 그래서 **이미 만료시킨 점유에 뒤늦은 확정이 닿는 레이스**가 구조적으로 존재한다. 이 레이스를 어떻게 막느냐가 사가의 정확성 핵심이다.

- **만료된 점유에는 확정을 거부한다 — 오버부킹 방지.** `SeatReleased`로 풀린 슬롯에 `ConfirmReservation`/`HoldSeat`이 뒤늦게 닿으면 `timetable` 애그리거트가 거부한다 — 만료 권위가 `timetable`에 있으므로(앞 절), "이 점유가 살아 있나"는 그 애그리거트의 불변식으로 동기 검증된다. 만료 점유에 확정을 허용하면 같은 슬롯이 두 번 팔리는 오버부킹이 된다. PM은 이 거부를 사가 실패로 받아 보상 경로로 들어간다.
- **paid-after-expiry — 돈만 받고 좌석은 없는 레이스.** 폴링이 `SeatReleased`를 발행한 *뒤에* `PaymentConfirmed`가 도착할 수 있다(결제는 이미 외부 PG에서 일어났고 점유는 그사이 만료). 이때 위 규칙대로 확정은 거부되지만 돈은 이미 받았다 — 좌석 없는 결제가 남는다. PM은 이 상태(점유 해제됨 + 결제 확정됨)를 감지해 **환불 보상(`PaymentRefunded`)을 트리거**한다. 즉 `SeatReleased`로 닫힌 사가라도 늦은 `PaymentConfirmed`를 무시해선 안 되고, 그것을 보상 트리거로 받아야 한다. (환불 명령이 결제 ACL을 어떻게 호출하는가는 [[RFC-016-payment-integration-boundary]]·[[14-payment-integration]] — PM은 보상 *명령*만 소유한다.)
