...
단일 해법은 없고 부수효과의 성격을 따라 갈래가 나뉜다. 기본값은 **"모든 비-멱등 부수효과는 inbox나 outbox로 감싼다"**이고, 수동 보정은 예외다.

- **inbox(기본)**: Kafka에서 받은 이벤트를 `event_id` + 페이로드 + 상태(PENDING/DONE/FAILED)로 inbox 테이블에 기록. 이미 처리한 `event_id`면 스킵. 프로젝터(§4)와 동일 패턴 — Outbox(발신) ↔ Inbox(수신) 대칭. 페이로드를 보존하므로 DLQ 수동 재생(§9) 시 inbox에서 꺼내 재처리할 수 있다.
