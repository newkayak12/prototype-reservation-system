...
- **병렬 상한 = 파티션 수.** 인스턴스가 파티션보다 많으면 초과분은 놀고만 있는다. 이것이 §4.3 에서 "파티션 수를 넉넉히"의 또 다른 이유다. — **단, 이 상한은 competing-consumers 모델에 한정된다.** 파티션을 넘어 확장이 필요하면 아래 Parallel Consumer로 상한을 파티션 수에서 분리한다.
- 순서 보장은 여전히 파티션 단위 → **같은 `aggregate_id` 는 한 인스턴스가 순서대로** 처리한다. 병렬화해도 애그리거트별 순서는 안전하다.

#### Parallel Consumer (파티션 내 key 병렬) — 채택된 프로젝션 컨슈머 모델

competing consumers 의 "병렬 상한 = 파티션 수"는 파티션 하나를 인스턴스 하나가 통째로 소비하기 때문이다. **Confluent Parallel Consumer 를 `processingOrder: KEY` 로 쓰면 한 파티션 안에서 key(= `aggregate_id`) 단위로 병렬 처리**한다 → 병렬 상한이 **파티션 수가 아니라 서로 다른 `aggregate_id` 수**가 된다. 파티션 수는 순서 계약(§4.3)을 위해 작게 고정해도 처리량은 컨슈머 동시성 설정으로 온라인 조절된다. 이로써 §4.3 "파티션 증설 = 새 토픽 마이그레이션"의 고비용 탈출구에 의존할 필요가 사라진다(파티션 초기 추정이 틀려도 동시성으로 흡수).

- **V1 선례.** V1 이 이미 `ParallelStreamProcessor` + `processingOrder: KEY`(key = `timeTableId_timeTableOccupancyId`) + `closeDrainFirst(30s)` 로 이 패턴을 운영했다([[01-v1-architecture-analysis]]). 새 메커니즘이 아니라 검증된 자산의 계승.
- **전달 보장 = at-least-once + 멱등 (EOS 아님).** Parallel Consumer 는 오프셋을 순서 무관하게 커밋하므로 크래시·리밸런싱 시 일부 오프셋이 재전달된다. 이 중복은 §4.5 의 event_id inbox 멱등 + 멱등 upsert 가 흡수한다. **V1 은 EOS(exactly-once) 모드를 썼으나 V2 는 채택하지 않는다** — 우리 애플리케이션 멱등이 이미 정확성을 보장하므로, 트랜잭션 프로듀서·오프셋 트랜잭션 결합의 EOS 복잡도는 기능이 겹치는 이중 방어다.
- **key 순서만 보장 = 정확성 요구와 정확히 일치.** 프로젝터 정확성 요구는 애그리거트별 순서 + 멱등 upsert + 버전 가드(ADR-22)이고, 교차-애그리거트 순서는 정확성 요구가 아니다. key = `aggregate_id` 이므로 `KEY` ordering 이 이 요구를 정확히 만족한다.
- **통합.** Spring `@KafkaListener` 리스너 컨테이너가 아니라 별도 poll 루프(V1 이 `@PostConstruct` subscribe/poll 로 검증). in-flight 상한·동시성은 튜닝 대상이고, poison message 는 §4.10 DLQ/꼬리-격리로 이관.
- **남는 별개 축 — 단일 프라이머리 DB 쓰기.** Parallel Consumer 는 소비 병렬성 상한만 올린다. 프로젝션 *쓰기* 는 여전히 단일 프라이머리로 몰리므로, DB 쓰기가 병목이면 배치 쓰기·read model key 샤딩·수직 확장이 별개 레버다. key 순서 보장이라 같은 aggregate 는 직렬 → 행 락 자기경합은 없고, 경합은 커넥션·IOPS 수준. 무트래픽이라 이 축은 이월.
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-07-05 | §4.4 **Parallel Consumer(`KEY` ordering) 채택** — 프로젝션 컨슈머 모델로 명시(V1 선례). 병렬 상한을 파티션 수에서 분리, at-least-once + 멱등(EOS 불채택). §268 Weakness(파티션 벌칙 극대화) 종결, competing-consumers 상한 줄에 한정 주석. 트리아지 C08 종결·C28 무력화. 단일 프라이머리 DB 쓰기는 별개 이월 축. |
...
- **파티션 수 = 계약이면 초기 추정 실패의 대가가 토픽 마이그레이션** — §4.3은 파티션 증설을 in-place로 금지하고 "새 토픽으로 마이그레이션(드레인→컷오버)"을 강제한다. 이는 파티션 초기값 추정이 틀리는 순간(트래픽 추정이 빗나가는 건 프로토타입에서 상수) 병렬성 상한(§4.4 병렬 상한=파티션 수)에 막힌 프로젝터가 lag을 쌓는데, 유일한 해법이 무중단 토픽 마이그레이션이라는 고비용 운영이 된다는 뜻이다. §7은 이를 "완화"로 적었지만 실제로는 초기 실수의 벌칙을 최대치로 키운 설계다 — 보수적 초기값(3/6~12)의 근거도 처리량 추정 TBD에 걸려 있어 순환이다.
  > **✅ 종결 (2026-07-05, 트리아지 C08/C28):** "병렬 상한 = 파티션 수"라는 전제가 competing-consumers 한정임을 §4.4 에 명시하고, **Parallel Consumer(`KEY` ordering, V1 선례)** 를 프로젝션 컨슈머 모델로 채택했다. 파티션 내 key 병렬로 상한이 파티션 수에서 분리되므로, 초기 파티션 추정이 틀려도 고비용 토픽 마이그레이션이 아니라 컨슈머 동시성으로 흡수된다 — 벌칙 극대화·순환 논리 해소. 단일 프라이머리 DB 쓰기는 별개 이월 축(§4.4).
