...
- **PII 셰딩과 콜드 경로**: 콜드로 빠진 이벤트도 키 삭제 *한 번*으로 같이 셰딩돼야 하므로([[RFC-005-pii-security]]), 콜드 스토리지에는 이벤트 본문(암호문)만 두고 **셰딩 키는 콜드로 함께 이관하지 않는다** — 키는 핫 경로의 전용 키 저장소에 남긴다. 그러면 키 저장소에서 키를 지우는 한 번의 동작이 핫·콜드를 가리지 않고 셰딩을 성립시킨다(키/blind index 본체는 [[11.es-pii-crypto-shredding]]). 콜드 본문 복호 시 핫 키 저장소 참조 결합도는 Design에서 검증 — **TBD**.
