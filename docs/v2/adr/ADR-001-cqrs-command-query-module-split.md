...
V1은 `core-module` / `application-module` / `adapter-module` 단일 계층 구조에서 읽기와 쓰기 코드가 한 모델·한 DB를 공유했다([[DESIGN-001-design-overview]]). V2의 목표는 CQRS — 읽기/쓰기 경로를 분리해 각자 독립적으로 진화·확장하게 만드는 것이다.
...
- 모듈 경계는 Gradle로 1차 강제. 상세 트리·서브모듈 구조는 [[DESIGN-002-module-structure]].

### 결과 (Consequences)

- 좋은 점: 읽기/쓰기가 컴파일 단위로 격리된다 / query가 command 도메인을 import할 경로가 없다 / `query-module`을 별도 배포로 떼기 쉽다([[DESIGN-010-deployment-runtime]]).
...
도메인 패키지 간 경계는 Konsist 아키텍처 규칙으로 강제([[DESIGN-012-environments-and-testing]]) — `query`가 `command`/core를 참조하면 빌드 실패.

## 추가 정보 (More Information)

- **미결정 (→ 구현 사이클)**: query를 물리적으로 분리할 시점·기준(현재 단일 배포).
- 관련: [[RFC-001-v2-cqrs-and-event-sourcing]] · [[DESIGN-002-module-structure]] · [[ADR-003-command-hexagonal-query-layered]]
